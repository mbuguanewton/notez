...
const normalizeHtml = (html: string): string => {
  if (!html) return "";
  // Much lighter normalization - only normalize line endings and trim
  // Preserve intentional whitespace in content
  return html.replace(/\r\n/g, "\n").trim();
};
  ...
  (
    ...
    const editor = useEditor({
      ...
      onUpdate: ({ editor }) => {
        ...
        console.log("Editor onUpdate fired:", {
        ...
        debounceTimeoutRef.current = setTimeout(() => {
          console.log(
            "Debounced onChange call with content length:",
            html?.length,
          );
          if (onChange) {
            onChange(html);
          } else {
            console.warn("onChange callback is not available");
          }
        }, 150);
      ...
      editorProps: {
        attributes: {
          class:
            "prose prose-lg dark:prose-invert max-w-none focus:outline-none min-h-[500px] px-0",
        },
    ...
    useEffect(() => {
      if (editor && content !== undefined) {
        ...
        console.log("Content useEffect triggered:", {
          ...
          areEqual: content === currentEditorContent,
        });

        // Always update the editor if content is different
        if (content !== currentEditorContent) {
          console.log("Updating editor with new content");

          // Use a timeout to ensure the update happens after current render cycle
          setTimeout(() => {
    ...
    useImperativeHandle(
      ...
      () => ({
        ...
        getContent: () => {
          return editor ? editor.getHTML() : contentRef.current;
        },
      }),
      [editor, onChange],
    );
